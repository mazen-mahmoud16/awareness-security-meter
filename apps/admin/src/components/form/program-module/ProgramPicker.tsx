...
import { programNamesForTenantQuery, programQuery } from "../../../api/program";
import AuthenticatedImage from "../../file/AuthenticatedImage";
import InputLabel from "../../UI/InputLabel";

const ProgramPicker: React.FC<{ disabled: boolean }> = ({ disabled }) => {
  const { watch, setValue } = useFormContext();
  const id = watch("program");
  const tenantId = useParams().id!;
  const [search, setSearch] = useState("");
  const [isTenantSpecific, setIsTenantSpecific] = useState(false);
  const { data, isFetching } = useQuery(
    programNamesForTenantQuery(
  ...
  const { data: programData } = useQuery(
    programQuery(id, inOptions === undefined && id !== undefined)
  );
  ...
  const value =
    ...
    (programData
      ? {
          image: programData?.result.thumbnailImage,
          value: programData?.result.id,
          label: programData?.result.name,
        }
      : undefined);
  ...
  return (
    <div>
      <div className="flex justify-between">
        <h2 className="font-semibold text-2xl pb-4">Pick Program</h2>
