...
import { videosQuery } from "../../../api/content";
...
const VIDEO_UPLOAD_PATH = "/admin/upload/video";
const ITEMS_PER_PAGE = 15;

const VideoExplorer: React.FC<Props> = ({ onClose, initialSelection }) => {
  const [page, setPage] = useState(0);
  const [search, setSearch] = useState("");
  const { data, isLoading, isPreviousData } = useQuery(
    videosQuery(page * ITEMS_PER_PAGE, ITEMS_PER_PAGE, search)
  );
  ...
  return (
    <div className="flex flex-col h-full w-full justify-start items-start">
      ...
      <h1 className="text-2xl font-bold pb-6">Videos</h1>
      <Input
        className="mt-2 mb-8"
