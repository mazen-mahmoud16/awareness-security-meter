...
import { tenantModulesQuery } from "../../../../api/tenant/module";
...
const Modules: React.FC<Props> = () => {
  ...
  return (
    <div className="pt-4">
      <Helmet>
        <title>{tenantData?.result.name || ""} | Modules</title>
      </Helmet>
      <div className="flex justify-between items-center">
        <h1 className="font-bold text-3xl pb-4">Exposed Modules</h1>
      </div>
      {isLoading && <CenteredSpinner />}
      {data?.result.length === 0 && (
        <EmptyListMessage entity="Exposed Modules" />
      )}
      {data?.result.map((m) => (
