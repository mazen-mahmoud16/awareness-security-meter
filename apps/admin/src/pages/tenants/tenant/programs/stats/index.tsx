...
import { deleteProgram, programQuery } from "../../../../../api/program";
import {
...
const ProgramStats: React.FC<Props> = () => {
  const { id, pid } = useParams();
  const { data, isLoading, isError } = useQuery(programQuery(pid!));
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const { mutate: deleteMutation } = useMutation(
    async () => deleteProgram(pid!),
    {
      async onSuccess() {
        await queryClient.invalidateQueries(["programs", id]);
        navigate(`/tenants/${id}/`);
      },
    }
  );
    ...
    <div className="pt-4">
      <div className="flex justify-between items-center">
        <div className="flex items-center">
          <BsFillInboxesFill size={28} className="text-primary-500" />
...
const ProgramUsers = ({ id, tenant }: { id: string; tenant: string }) => {
  const [page, setPage] = useState(0);
  const [isModalOpen, setIsModalOpen] = useState(false);
  var { data, isLoading, isPreviousData } = useQuery(
    programUsersQuery(id, tenant, {
  ...
  const queryClient = useQueryClient();
  const { mutate: deleteMutation } = useMutation(
    async (user: string) => await deleteProgramSession(id, user, tenant),
    {
      async onSuccess() {
